...
import { defaultLocale, getCategoryDescription, localizeDocumentTitle } from '@/lib/i18n'
...
export default async function Pages({ params }: PageProps) {
  ...
  if (route?.noLink && route.items) {
    const title = localizeDocumentTitle(routeHref, route.title, defaultLocale)
    const description = getCategoryDescription(routeHref, defaultLocale)
    ...
    return (
      <div className="flex w-full min-w-0 items-start gap-6">
        <section className="mb-6 min-w-0 flex-1 rounded-none border-y bg-card px-5 py-6 sm:rounded-lg sm:border sm:px-8 sm:py-8 lg:px-10">
          <ArticleBreadcrumb paths={slug} locale={defaultLocale} />
  ...
  const res = await getDocument(pathName, defaultLocale)
  ...
  return (
    <div className="flex w-full min-w-0 items-start gap-6">
      <section className="mb-6 min-w-0 flex-1 rounded-none border-y bg-card px-5 py-6 sm:rounded-lg sm:border sm:px-8 sm:py-8 lg:px-10">
        ...
        <div className="flex flex-col gap-3">
          <h1 className="text-2xl leading-tight font-semibold tracking-[-0.02em] text-foreground sm:text-[28px]">
            {localizeDocumentTitle(`/${pathName}`, frontmatter.title, defaultLocale)}
          </h1>
        ...
        <Typography>
          ...
          <Pagination pathname={pathName} locale={defaultLocale} />
        </Typography>
      </section>
      <TableOfContents
        tocs={{ tocs }}
