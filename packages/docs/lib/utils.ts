...
import searchJsonEn from '@/public/search-data/documents.en.json'
import searchJson from '@/public/search-data/documents.json'
import searchJsonTh from '@/public/search-data/documents.th.json'
...
const searchDataByLocale: Record<Locale, SearchDocument[]> = {
  en: (searchJsonEn.length ? searchJsonEn : searchJson) as SearchDocument[],
  th: searchJsonTh as SearchDocument[],
}
...
export function advanceSearch(query: string, locale: Locale = 'en') {
  const lowerQuery = query.toLowerCase().trim()
  const queryWords = lowerQuery.split(/\s+/).filter((word) => word.length >= 3)

  if (queryWords.length === 0) return []

  const searchData = searchDataByLocale[locale] ?? searchDataByLocale.en
